...
const Qualification = () => {
  ...
  return (
    <section className="bg-background py-12 sm:py-20" ref={ref}>
      <div className="container mx-auto px-5 sm:px-4 lg:px-8 max-w-3xl">
        ...
        <div className={`bg-card rounded-xl border border-border p-5 sm:p-8 mb-5 sm:mb-8 transition-all duration-700 delay-200 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <p className="font-heading font-bold text-sm sm:text-lg text-primary mb-4 sm:mb-6 uppercase tracking-wider">
            É para você se:
          </p>
          <ul className="space-y-3 sm:space-y-4">
            {criteria.map((c, idx) => (
              ...
              >
                ...
                <span className="text-foreground font-medium text-xs sm:text-base">{c}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className={`text-center transition-all duration-700 delay-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <a href={WA_LINK} target="_blank" rel="noopener noreferrer">
            <Button
              size="lg"
              className="bg-primary text-primary-foreground font-heading font-bold text-sm sm:text-base px-8 sm:px-10 py-5 sm:py-6 hover:bg-primary/90 hover:scale-105 transition-all hover:shadow-[0_0_30px_rgba(255,215,0,0.3)] w-full sm:w-auto active:scale-95"
            >
