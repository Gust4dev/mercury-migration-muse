...
import { ArrowRight, Layers, TrendingUp, Tag } from "lucide-react";
...
const solutions = [
  {
    icon: Layers,
    title: "Mercury Nexus",
    desc: "Plataforma completa de atendimento, CRM, automação comercial e remarketing para empresas que desejam crescer com eficiência.",
    cta: "Conhecer plataforma",
    href: "#nexus",
    external: false,
    highlights: ["IA", "CRM", "WhatsApp", "Remarketing"],
    sectors: ["Plataforma Principal"],
  },
  {
    icon: TrendingUp,
    title: "Vendi.Mais",
    desc: "Transforme seu WhatsApp em um canal completo de vendas com catálogo, carrinho, pagamentos e automações.",
    cta: "Conhecer solução",
    href: "/vendimais",
    external: false,
    isRoute: true,
    highlights: ["E-commerce", "WhatsApp", "Vendas", "Automação"],
      ...
      <div className="container mx-auto px-5 sm:px-4 lg:px-8">
        <div className={`text-center mb-10 sm:mb-16 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          ...
          <h2 className="font-heading text-2xl sm:text-3xl md:text-4xl font-bold text-foreground mb-3 sm:mb-4">
            A plataforma e a solução que <span className="text-primary">aceleram seu crescimento</span>
          </h2>
          <p className="text-muted-foreground max-w-2xl mx-auto text-sm sm:text-base px-2">
            A Mercury Nexus é nossa plataforma principal. O Vendi.Mais complementa o ecossistema para vendas via WhatsApp.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-8 max-w-4xl mx-auto">
          {solutions.map((sol, idx) => {
            ...
            const cardClass = `relative bg-card rounded-2xl border border-border p-5 sm:p-8 hover:border-primary/40 hover:-translate-y-1 hover:shadow-xl transition-all duration-500 group overflow-hidden active:scale-[0.99] block ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`;
            const style = { transitionDelay: `${300 + idx * 150}ms` };

            if (sol.isRoute) {
              return (
                <Link key={sol.title} to={sol.href} className={cardClass} style={style}>
                  {CardInner}
                </Link>
              );
            }
            return (
              <a
                key={sol.title}
                href={sol.href}
                onClick={(e) => handleAnchor(e, sol.href)}
                className={cardClass}
                style={style}
              >
                {CardInner}
